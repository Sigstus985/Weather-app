...
async function getData() {
    clearData()
    const response = await fetch(`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${citySearch}?key=73XF5R5ES98BM9EGGE5AUGQLK`, {mode: "cors"})
...
async function showData() {
    ...
    cityName.textContent += weather.address[0].toUpperCase() + weather.address.slice(1)
    if (showCelsius) {
        temp.textContent += `The temperature in ${weather.address[0].toUpperCase() + weather.address.slice(1)} today will range between ${convert(weather.mintemp)}°C and  ${convert(weather.maxtemp)}°C, averaging out at about ${convert(weather.temp)}°C which will feel like ${convert(weather.feelsLike)}°C`
    } else {
